...
createLinkstring = function (para) {
    var arg = "";
    for (i in para) {
        if ((!para[i] && typeof para[i] != 'mobile') || i == 'msg' || i == 'mobile' || i == 'outtime' || i == 'code' || i == 'price' || (i + '').toLowerCase() == 'creattime')continue; //跳过传递此类
...
callback = function (data) {
    ...
    if (!isNaN(code) && code == 1) {
        ...
        $("#msg h1").html("信息不完整,请勿非法提交。");
    } else if (data.code == -2) {
        $("#msg h1").html("签名验证失败,请勿非法提交。");
    } else if (data.code == -3) {
        $("#msg h1").html("商家的软件未对接此支付方式。");
    } else {
...
show_Qrcode = function (data) {
	...
	if (user_data && user_data.qrcode_url) {
       data.qrcode = user_data.qrcode_url + '?pid=' + data.pid + '&money=' + data.price + '&type=' + data.type;
    }
    if (data.qrcode)$("#show_qrcode").attr("src", data.qrcode); //二维码更新
    if (data.price) $("#price").html('￥' + data.price); //金额改变 云端匹配最佳金额
    var tps = data.price != data.money ? '<span style="color:red">为了您及时到账 请务必付款' + data.price + '元</span><br>' : '';
    if (data.mobile) {
        if (data.type == 'alipay') {
			$(".alipayApp").attr('style','');
          tps += '长按保存到相册使用"支付宝扫一扫"扫码';
			...
			if(isAndroid){
				...
				$(".ioswxpayApp").attr('style','');
			}
           tps += '长按保存到相册使用"微信扫一扫"扫码';
        } else{
           tps += '长按保存到相册使用"QQ扫一扫"扫码';
        }
        // $("#msg h1").html(tps); //手机支付
    } else {
        tps += tps ? '' : '请使用手机扫描上方二维码 并支付'+$("#price").html()+'元<br>不要多付或者少付 否则不会到账';
...
show_desc = function (data) { //商品描述
    var html = '';
    html += getDescMode('云端单号', data.trade_no);
    html += getDescMode('商户单号', data.out_trade_no);
    html += getDescMode('用户的IP', pay_id);
    html += getDescMode('商品名称', data.name);
    html += getDescMode('付款金额', "￥" + data.price);
...
var notifyServer
startNotiry = function (data) {
	var query = createLinkstring(data);	
	var reslut_url = 'pay.php?act=notify&'+query;
	 Notiry = window.setInterval(function () {
		$.post(reslut_url,'',function(json){
					// alert(json);
				if(json.indexOf('success') >= 0){
					$("#show_qrcode").attr("src", '/assets/zeropay/assets/img/pay_ok.png');
					$("#show_qrcode").attr("alt", '支付成功 2秒后自动跳转');
					$("#msg h1").html('支付成功 2秒后自动跳转'); 
					var openUrl = user_data.return_url;
					openUrl += openUrl.indexOf('?') >= 0 ? "&" + query : "?" + query;
					setTimeout(function () {
...
//var t1 = window.setInterval(startNotiry,4000); //继续查询
